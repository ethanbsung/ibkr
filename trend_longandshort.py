...
# Strategy Parameters
short_moving_avg_period = 20  # Short period for the moving average
long_moving_avg_period = 50   # Long period for the moving average
stop_loss_points = 6  # Stop loss in points
take_profit_points = 16  # Take profit in points
...
bars = ib.reqHistoricalData(
    ...
    durationStr='6 M',  # 12 months of data
    barSizeSetting='15 mins',  # 15-minute bars
    whatToShow='TRADES',
...
position_size = 0  # 1 = long, -1 = short, 0 = no position
entry_price = None
cash = initial_cash
trade_results = []
daily_returns = []
...
for i in range(long_moving_avg_period, len(df)):
    ...
    if position_size == 0:  # No position open
        # Enter long position when short MA crosses above long MA
        if short_moving_avg > long_moving_avg:
            position_size = 1  # Enter long
        ...
        # Enter short position when short MA crosses below long MA
        elif short_moving_avg < long_moving_avg:
            position_size = -1  # Enter short
            entry_price = current_price
            take_profit_level = entry_price - take_profit_points
            stop_loss_level = entry_price + stop_loss_points
            print(f"Short entry: {position_size} contract at {entry_price:.2f}")
            print(f"Take profit level: {take_profit_level:.2f}, Stop loss level: {stop_loss_level:.2f}")

    elif position_size == 1:  # Long position open
        ...
        if current_low <= stop_loss_level:
            ...
            total_commissions += commission_per_side * 2
            cash += pnl
        ...
        # Exit long and go short if short MA crosses below long MA
        elif short_moving_avg < long_moving_avg:
            ...
            trade_results.append(pnl)
            daily_returns.append(pnl / cash)
            print(f"Exit long: {position_size} contract at {current_price:.2f} (PnL: {pnl:.2f})")
            position_size = -1  # Enter short position
            entry_price = current_price
            take_profit_level = entry_price - take_profit_points
            stop_loss_level = entry_price + stop_loss_points
            print(f"Short entry: {position_size} contract at {entry_price:.2f}")
    ...
    elif position_size == -1:  # Short position open
        # Check for stop-loss hit
        ...
        # Exit short and go long if short MA crosses above long MA
        elif short_moving_avg > long_moving_avg:
            pnl = (current_price - entry_price) * position_size * contract_multiplier - (commission_per_side * 2)
            total_commissions += commission_per_side * 2
            cash += pnl
            trade_results.append(pnl)
            daily_returns.append(pnl / cash)
            print(f"Exit short: {position_size} contract at {current_price:.2f} (PnL: {pnl:.2f})")
