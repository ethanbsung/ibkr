#!/usr/bin/env python3
"""
IBKR live portfolio ledger — EWMAC + dynamic portfolio optimisation.

A strategy-agnostic ledger (the dyn-opt executor in ibkr_fut/live_dynamic.py is its
first user, but nothing here is EWMAC-specific). Mirrors paper/ibkr_logger.py but
drops the IBS-specific fields and takes the contract multiplier per fill so any
instrument can be logged.

Ledger files in paper/ledgers/ibkr_dynamic/:
  daily.csv      — daily account snapshot + rolling performance
  trades.csv     — per-fill: signal px, fill px, slippage, commission, forecast
...
Integration:
    ...
    ledger = DynLedger()
    ledger.log_fill(...)      # after each confirmed fill
    ledger.log_daily(ib)      # once at end of the daily run

Standalone report:
    python3 paper/dyn_ledger.py
"""
...
LEDGER_ROOT    = os.path.join(HERE, "ledgers", "ibkr_dynamic")
...
TRADE_COLS = [
    ...
    "forecast", "signal_price", "fill_price", "fill_value",
...
IB_HOST, IB_PORT, CLIENT_ID = "127.0.0.1", 4002, 12
...
class DynLedger:
    ...
    def log_fill(
        self, *, symbol, contract, action, qty, multiplier,
        signal_price, fill_price, commission, forecast=None,
    ):
        """
        Record one confirmed fill. Call immediately after ib.sleep() post-order.

        symbol       — IBKR symbol (e.g. 'MES', 'ZC')
        contract     — contract month YYYYMM
        action       — 'BUY' or 'SELL'
        qty          — number of contracts filled (always positive)
        multiplier   — contract multiplier (IBKR), for value/slippage in USD
        signal_price — close price used for sizing (in IBKR price units)
        ...
        forecast     — combined EWMAC forecast that drove the target (diagnostic)
        """
        fill_value = fill_price * qty * multiplier
        sign       = 1 if action == "BUY" else -1
        slip_usd   = sign * (fill_price - signal_price) * qty * multiplier
        ...
        row = {
            ...
            "forecast":     round(forecast, 2) if forecast is not None else "",
...
def report():
    ...
    _emit()
    _emit("=" * W)
    _emit(f"  IBKR LIVE PORTFOLIO   EWMAC · dynamic optimisation")
    _emit(f"  as of {datetime.now().strftime('%Y-%m-%d  %H:%M')}")
    _emit("=" * W)

    daily = _read_csv(DAILY_PATH, DAILY_COLS)
    if daily.empty:
        _emit("\n  No history yet — run live_dynamic.py --execute to start logging.\n")
        _emit("=" * W)
    ...
    if not trades_df.empty:
        _hdr("Recent Trades  (last 12)  [fill px & commission = IBKR exact]")
        _emit(f"  {'Date':<12} {'Sym':<6} {'Act':<5} {'Qty':>3}  "
              f"{'Fcast':>6}  {'Signal Px':>11}  {'Fill Px':>11}  "
              f"{'Slip $':>8}  {'Comm':>6}")
        for _, r in trades_df.tail(12).iloc[::-1].iterrows():
            slip = float(r["slippage_usd"])
            slip_sign = "+" if slip >= 0 else ""
            fc = r["forecast"]
            fc_s = f"{float(fc):>6.1f}" if str(fc) not in ("", "nan") else f"{'':>6}"
            _emit(f"  {str(r['date']):<12} {str(r['symbol']):<6} {r['action']:<5} {int(r['qty']):>3}  "
                  f"{fc_s}  {float(r['signal_price']):>11,.4f}  "
                  f"{float(r['fill_price']):>11,.4f}  "
                  f"{slip_sign}${abs(slip):>6.2f}  ${float(r['commission']):>5.2f}")
