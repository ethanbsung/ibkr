...
df_1h = df_intraday_filtered.resample('1H').agg({
    'Open': 'first',
    'High': 'max',
    'Low': 'min',
    'Close': 'last',
...
print("\nResampled 1-Hour Data Range:")
print(df_1h.index.min(), "to", df_1h.index.max())
print("Number of Rows After Resampling:", len(df_1h))

# --- Compute Rolling High of Last 5 1-hour bars (excluding current bar) ---
rolling_window = 5  # Equivalent to 5 hours
df_1h['Rolling_High'] = df_1h['High'].shift(1).rolling(window=rolling_window, min_periods=rolling_window).max()

# Remove rows where rolling high is NaN
df_1h.dropna(subset=['Rolling_High'], inplace=True)

print("\nData Range After Rolling High Calculation:")
print(df_1h.index.min(), "to", df_1h.index.max())
print("Number of Rows After Rolling High Calculation:", len(df_1h))

# --- Verify Presence of Early Data ---
if df_1h.empty:
    logger.error("No data available after applying rolling high calculation. Check your data and rolling window.")
    exit(1)

# --- Check for Missing Data ---
expected_freq = '1H'  # Changed from '30min' to '1H' to avoid FutureWarning

# Generate the full expected index based on the resampled data
full_index = pd.date_range(start=df_1h.index.min(), end=df_1h.index.max(), freq=expected_freq)

# Find missing dates by comparing the full index with the resampled data's index
missing_dates = full_index.difference(df_1h.index)

print(f"\nNumber of Missing 1-Hour Bars: {len(missing_dates)}")
...
print("Resampled Data Timezone:", df_1h.index.tz)

# --- Initialize Backtest Variables ---
cash = INITIAL_CASH  # Initial cash remains unchanged
trade_results = []
balance_series = [INITIAL_CASH]
balance_dates = [df_1h.index.min()]
position = None  # To track if a position is open
...
# --- Backtesting Loop ---
for idx, (current_time, row) in enumerate(df_1h.iterrows()):
    if position is None:
        ...
        # For 1-hour bars, the first bar starts at 09:30 and ends at 10:30, etc.
        # Adjust the time check accordingly
        # Since the bar ends at 10:30, the current_time represents the end of the bar
        bar_start_time = (current_time - timedelta(hours=1)).time()
        bar_end_time = current_time.time()
        
        # Define trading hours
        market_open = time(9, 30)
        market_close = time(16, 0)
        
        # Check if the bar overlaps with trading hours
        # Only enter if the bar is fully within trading hours
        # For example, the bar 03:30 PM - 04:30 PM overlaps beyond 04:00 PM
        # Adjust to exclude such bars
        if (bar_start_time >= market_open) and (bar_end_time <= market_close):
        ...
        # Manage existing position within the current 1-hour bar
        # Check if stop loss or take profit is hit
        current_high = row['High']
        current_low = row['Low']

        exit_time = current_time
        exit_price = row['Close']  # Use 'Close' as the exit price
        ...
        # --- Removed EOD Exit Condition ---
        # Previously, positions were exited at the end of the day if SL or TP were not hit.
        # This block has been removed as per the user's request.

        # Note: No additional exit conditions are added here.
        # Positions remain open until SL or TP is hit.
...
print("\nBalance DataFrame Range:")
print(balance_df.index.min(), "to", balance_df.index.max())
...
if not drawdown_periods.empty:
    end_dates = drawdown_periods.index.to_series().diff().ne(timedelta(hours=1)).cumsum()
    drawdown_groups = drawdown_periods.groupby(end_dates)
    drawdown_durations = drawdown_groups.size()
    max_drawdown_duration_days = drawdown_durations.max() * (1/24)  # 1 hour = 1/24 days
    average_drawdown_duration_days = drawdown_durations.mean() * (1/24)
else:
...
days = (df_1h.index.max() - df_1h.index.min()).days if (df_1h.index.max() - df_1h.index.min()).days > 0 else 1
...
initial_close = df_1h.iloc[0]['Close']
final_close = df_1h.iloc[-1]['Close']
benchmark_return = ((final_close - initial_close) / initial_close) * 100

# --- Performance Metrics ---
results = {
    "Start Date": df_1h.index.min().strftime("%Y-%m-%d"),
    "End Date": df_1h.index.max().strftime("%Y-%m-%d"),
    ...
    "Sharpe Ratio": f"{(balance_df['Equity'].pct_change().mean() / balance_df['Equity'].pct_change().std() * np.sqrt(252)) if balance_df['Equity'].pct_change().std() != 0 else 0:.2f}",
...
if len(balance_series) < 2:
    ...
    initial_close = df_1h.iloc[0]['Close']
    benchmark_equity = (df_1h['Close'] / initial_close) * INITIAL_CASH
    ...
    plt.title('Equity Curve: Strategy vs Benchmark (1-Hour Timeframe)')
    plt.xlabel('Time')
