...
DATA_SYMBOL = 'CL'           # E-mini S&P 500 for data
DATA_EXPIRY = '202502'       # Feb 2025
DATA_EXCHANGE = 'NYMEX'        # Exchange for ES

EXEC_SYMBOL = 'MCL'          # Micro E-mini S&P 500 for execution
EXEC_EXPIRY = '202502'       # Feb 2025
EXEC_EXCHANGE = 'NYMEX'        # Exchange for MES
CURRENCY = 'USD'

INITIAL_CASH = 5000          # Starting cash
POSITION_SIZE = 1            # Number of MCL contracts per trade

BOLLINGER_PERIOD = 15
BOLLINGER_STDDEV = 2
STOP_LOSS_DISTANCE = 0.05        # Points away from entry
TAKE_PROFIT_DISTANCE = 0.15     # Points away from entry
...
# --- Define Contracts ---
cl_contract = Future(symbol=DATA_SYMBOL, lastTradeDateOrContractMonth=DATA_EXPIRY, exchange=DATA_EXCHANGE, currency=CURRENCY)
mcl_contract = Future(symbol=EXEC_SYMBOL, lastTradeDateOrContractMonth=EXEC_EXPIRY, exchange=EXEC_EXCHANGE, currency=CURRENCY)

# Qualify Contracts
try:
    qualified_contracts = ib.qualifyContracts(cl_contract, mcl_contract)
    cl_contract = qualified_contracts[0]
    mcl_contract = qualified_contracts[1]
    logger.info(f"Qualified CL Contract: {cl_contract}")
    logger.info(f"Qualified MCL Contract: {mcl_contract}")
except Exception as e:
...
try:
    logger.info("Requesting historical CL data (including ETH)...")
...
def place_bracket_order(action, current_price):
    ...
    try:
        ...
        parent_trade = ib.placeOrder(mcl_contract, bracket[0])
        ...
        take_profit_trade = ib.placeOrder(mcl_contract, bracket[1])
        ...
        stop_loss_trade = ib.placeOrder(mcl_contract, bracket[2])
