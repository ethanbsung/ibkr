...
INITIAL_CASH = 5000
ES_MULTIPLIER = 3  # 1 ES point = $5 profit/loss per contract
STOP_LOSS_POINTS = 5
...
# --- Compute Rolling High of Last 10 30-minute bars (excluding current bar) ---
rolling_window = 16
df_30m['Rolling_High'] = df_30m['High'].shift(1).rolling(window=rolling_window, min_periods=rolling_window).max()
...
print(df_30m.index.min(), "to", df_30m.index.max())
print("Number of Rows After Rolling High Calculation:", len(df_30m))
...
expected_freq = '30min'  # Changed from '30T' to '30min' to avoid FutureWarning
...
for idx, (current_time, row) in enumerate(df_30m.iterrows()):
    if position is None:
        ...
        if (current_time.time() >= time(9, 30)) and (current_time.time() < time(16, 0)):
...
expected_return = np.mean(strategy_returns)
downside_deviation = np.std(downside_returns)
sortino_ratio = (expected_return - mar) / downside_deviation * np.sqrt(252) if downside_deviation != 0 else np.nan

# --- Calculate Calmar Ratio ---
annualized_return_percentage = ((cash / INITIAL_CASH) ** (365.0 / (end_time - start_time).days) - 1) * 100 if (end_time - start_time).days > 0 else 0.0
