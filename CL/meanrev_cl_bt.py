...
csv_file_5m = 'Data/cl_5m_data.csv'
csv_file_30m = 'Data/cl_30m_data.csv'
...
custom_start_date = "2019-01-01"  # Adjust based on your data
...
df_high_freq = df_5m  # Use 5-minute data
...
for timestamp, current_bar in df_high_freq.iterrows():
    ...
    if position_size == 0:
        # No open position, check for entry signals based on 30-minute data
        # Check if current_time aligns with a 30-minute bar
        ...
        # Position is open, check for exit signals using 5-minute data
        ...
        if exit_price is not None and exit_time is not None:
            ...
            if position_type == 'long':
                pnl = ((exit_price - entry_price) * CONTRACT_MULTIPLIER * position_size) - (COMMISSION * position_size * 2)  # Example: Spread cost
            elif position_type == 'short':
                pnl = ((entry_price - exit_price) * CONTRACT_MULTIPLIER * position_size) - (COMMISSION * position_size * 2)
    ...
    # Append current cash to balance_series at each 5-minute bar
...
balance_series = pd.Series(balance_series, index=df_high_freq.index)
