...
custom_start_date = "2024-01-29"  # Adjust based on your data
...
# Initialize lists to store trade entries and exits
trade_entries = []
trade_exits = []
...
last_breakout_price = None  # Initialize last breakout price to prevent multiple entries at the same level
...
for i in range(len(df_30m_rth)):
    ...
    if position_size == 0:
        ...
        breakout_price_long = lower_band + 1  # Buy 1 point above lower band
        breakout_price_short = upper_band - 1  # Sell 1 point below upper band

        eligible_for_entry = False

        # Determine if the current breakout price is higher than the last breakout price (for longs)
        # or lower (for shorts) to prevent multiple entries at the same level
        if current_price < lower_band and (last_breakout_price is None or breakout_price_long > last_breakout_price):
            eligible_for_entry = True
            trade_direction = 'long'
            breakout_price = breakout_price_long
        elif current_price > upper_band and (last_breakout_price is None or breakout_price_short < last_breakout_price):
            eligible_for_entry = True
            trade_direction = 'short'
            breakout_price = breakout_price_short
        ...
        if eligible_for_entry:
            if trade_direction == 'long':
                ...
                # Update last_breakout_price to prevent re-entry at the same level
                last_breakout_price = breakout_price
        ...
        if exit_price is not None and exit_time is not None:
            ...
            # Reset last_breakout_price to allow new entries
...
# --- Export Trade Entries and Exits to CSV ---
entries_df = pd.DataFrame(trade_entries)
exits_df = pd.DataFrame(trade_exits)

# Define CSV file paths
entries_csv = 'trade_entries.csv'
exits_csv = 'trade_exits.csv'

# Export to CSV
entries_df.to_csv(entries_csv, index=False)
exits_df.to_csv(exits_csv, index=False)

logger.info(f"Exported {len(entries_df)} trade entries to '{entries_csv}'.")
logger.info(f"Exported {len(exits_df)} trade exits to '{exits_csv}'.")
